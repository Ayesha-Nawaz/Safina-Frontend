...
function TabBar({ state, descriptors, navigation }) {
  const { fetchUser } = useContext(UserContext); // Add fetchUser from UserContext
    ...
    >
      {state.routes.map((route, index) => {
        ...
        const onPress = async () => {
          ...
          // Fetch user data when Home tab is pressed
          if (route.name === 'index') {
            try {
              await fetchUser();
            } catch (error) {
              console.error("Error fetching user on Home tab press:", error);
            }
          }
        };
  ...
  return (
    <Tabs
      ...
      screenOptions={({ route }) => ({
        ...
        headerTitle: () => <CustomHeaderTitle route={route} />, // Apply to all screens
