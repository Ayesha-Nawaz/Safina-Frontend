...
export default function EditAsmaulhusna() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(`${BASE_URL}/asmaulhusna/${item.number}`, formData);
      
      Alert.alert(
        "Success",
        "Asmaul Husna updated successfully",
        [{ text: "OK", onPress: () => navigation.goBack() }]
      );
    } catch (error) {
      console.error('Error updating Asmaul Husna:', error);
      Alert.alert("Error", "Failed to update Asmaul Husna");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Edit Asmaul Husna</Text>
